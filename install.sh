...
if [ ! -d "$dir" ] ; then
    ...
    (sudo apt-get install -qq -y git python3-pip libgeos-c1v5 libatlas-base-dev libopenblas-dev python3-venv libssl-dev > /dev/null) & spinner
    wait
    echo ""
    echo "Downloading Web Plotter from Github"
    git clone -q -b Pi_OS_Bullseye64 https://github.com/ithinkido/penplotter-webserver.git $dir > /dev/null
    wait
    echo ""
    ...
    git clone -q -b Pi_OS_Bullseye64 https://github.com/ithinkido/penplotter-webserver.git $HOME/webplotter > /dev/null &&
    # add user files back
    rm -R $dir/uploads
    mv -f temp/* $dir/
    #clean up
    rm -R temp
    . penplotter_ws/bin/activate
